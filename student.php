...
<!-- general form elements -->
            ...
            <form role="form" method="POST" >
              <div class="box-body">
                 ...
                 <div class="form-group">
                  ...
                  <input name="sid" type="text" class="form-control" id="exampleInputPassword1" placeholder="Enter Student No" required>
                </div>
 ...
 <div class="form-group">
                <label>Course</label>
                <select class="form-control select2 select2-hidden-accessible" style="width: 100%;" tabindex="-1" aria-hidden="true" name="classroom"><option >Select Course</option>
                  <?php
                  ...
                  if ($result->num_rows > 0) {
                   // output data of each row
                     while($row = $result->fetch_assoc()) {
                      ...
                      echo "<option value='".$row["pid"]."' >".$row["fname"]." ".$row["lname"]." :".$row["pid"]."</option>";
                       }
                                  }
              ...
              if (isset($_POST['submit'])) {
                ...
                $sql = "SELECT * FROM subject WHERE sid='".$classroom."'  ";
                  $resultfe = $conn->query($sql);
                  if ($resultfe->num_rows > 0) {
                   // output data of each row
                     while($row = $resultfe->fetch_assoc()) {

                      $fees = $row['description'];
              ...
              // echo $total_balance;
                  ...
                  try {
                   ...
                   $insert = mysqli_query($conn,"INSERT INTO fees(sid,sub_id,total_billed,total_paid,total_balance)VALUES('".$sid."','".$classroom."','".$total_billed."','".$total_paid."','".$total_balance."')")or die (mysqli_error());
 ...
 <div class="form-group">
                ...
                <select class="form-control select2 select2-hidden-accessible" style="width: 100%;" tabindex="-1" aria-hidden="true" name="classroom"><option >Select Class Room</option>
                  ...
                  if ($result->num_rows > 0) {
                   // output data of each row
                     while($row = $result->fetch_assoc()) {
                  echo "<option "; if($classroom==$row["sid"]){echo 'selected="selected"';} echo " value='".$row["sid"]."' >".$row["title"]."_ID:".$row["sid"]."</option>";
                       }
                        }
                  ?>
                </select>
                  ...
                  if ($resultfe->num_rows > 0) {
                   // output data of each row
                     while($row = $resultfe->fetch_assoc()) {
                      ...
                      $total_billed = $fees;
                      $total_balance = $fees;
                     $updateEmp=mysqli_query($conn,"UPDATE fees SET sub_id='$classroom',total_billed='$total_billed',total_paid='$total_paid',total_balance='$total_balance' WHERE sid='$sid' " )or die (mysqli_error()); 
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  ...
                  if ($result->num_rows > 0) {
                   // output data of each row
                     while($row = $result->fetch_assoc()) {
                      echo "<tr><td> " . $row["sid"]. " </td><td> " . $row["fname"]." ". $row["lname"]. " </td><td> " . $row["bday"]. "</td><td>" . $row["gender"]. "</td><td>" . $row["address"]. "</td>
                      <td>" . $row["classroom"]. "</td>
                      <td>" . $row["parent"]. "</td>

                      <td><a href='student.php?update=". $row["sid"]."'><small class='label  bg-orange'>Update</small></a></td>
                      </tr>";
                       }
                                  }
